...
fn gcd(a: &Vec<i64>) -> i64 {
    if a.len() == 2 {
        return egcd(a[0], a[1]).2;
    }
    let gcd_rest = gcd(&a.iter().skip(1).copied().collect());
    return gcd(&vec![a[0], gcd_rest]);
}
...
fn egcd(a: i64, b: i64) -> (i64, i64, i64) {
    ...
    return (old_s, old_t, old_r); // (x, y, gcd, such that x * a[0] + y * a[0] == gcd)
}

fn bezout(numbers: &Vec<i64>, mut target: i64, max: i64) -> Vec<Vec<i64>> {
    if numbers.is_empty() {
        return vec![vec![]];
    }
    ...
    if reduced_numbers.len() == 2 {
        ...
        pairs = pairs
            .into_iter()
            .filter(|x| x.iter().all(|y| *y <= max && *y >= 0))
            .collect();
        // println!("{:?}", pairs);
        return pairs;
    }
...
fn row_reduce(
    ...
    let mut pivot_row_par = par_matrix_mut[pivot_row_idx].clone();
    ...
    for (idx, (row, par_row)) in matrix_mut
        .into_iter()
        .zip(par_matrix_mut.into_iter())
        .enumerate()
    {
        if idx < pivot_row_idx {
            ...
            match first_non_zero {
                Some(pos) => {
                    ...
                    if new_row[pos] < 0 {
                        new_row = new_row.into_iter().map(|x| x * -1).collect();
                        par_new_row = par_new_row.into_iter().map(|x| x * -1).collect();
                    }
                }
                None => {}
            };
...
fn row_reduce_updown(
    ...
    let mut new_matrix = vec![];
    let mut new_par_matrix = vec![];
    for (_idx, (row, par_row)) in matrix_mut
        .into_iter()
        .zip(par_matrix_mut.into_iter())
        .enumerate()
    {
...
fn smith_normal_form(switches: Vec<Vec<i64>>, joltage: Vec<i64>) -> (Vec<Vec<i64>>, Vec<Vec<i64>>) {
    ...
    // Eliminate entries
    return (switches_transpose, t_matrix);
}
...
fn recurse(mut switches: Vec<Vec<i64>>, mut remaining: Vec<i64>) -> Option<i64> {
    ...
    for presses in 0..=to_remove {
        ...
        match cur_pres {
            Some(number) => {
                min_presses = min_presses.min(presses + number);
            }
            None => {}
        }
